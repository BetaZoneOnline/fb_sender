...
class TaskEngine(QObject):
    ...
    def _start_worker(self, uid: UidRow) -> None:
        ...
        try:
            result = send_message(profile_ctx, uid.normalized_uid, self._config.page_load_countdown_sec)
        except Exception as exc:  # pragma: no cover - defensive safeguard
            self._active_worker = None
            self._handle_result(uid, "FAIL_RETRYABLE", "WORKER_EXCEPTION", str(exc), None)
            return
        self._handle_result(
            uid,
            result.status,
