...
from app.browser_host import BrowserHost
...
class UidManagementWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        right_layout.addWidget(self._build_browser_controls())
    ...
    def _build_browser_controls(self) -> QWidget:
        container = QWidget()
        layout = QVBoxLayout(container)
        title = QLabel("Browser window")
        title.setStyleSheet("font-weight: bold;")
        layout.addWidget(title)
        info = QLabel(
            "The Facebook browser session opens in a separate window. "
            "Use the button below to bring it to the front."
        )
        info.setWordWrap(True)
        layout.addWidget(info)
        self.show_browser_button = QPushButton("Show Browser Window")
        self.show_browser_button.clicked.connect(self._focus_browser)
        layout.addWidget(self.show_browser_button)
        layout.addStretch()
        return container
    ...
    def _ensure_browser_ready(self) -> None:
        storage_path = self._profile_manager.profile_storage_path
        self._browser_host.ensure_window(storage_path)

    def _focus_browser(self) -> None:
        self._ensure_browser_ready()
        self._browser_host.focus()
