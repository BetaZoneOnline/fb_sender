...
class FBWorker(QObject):
    ...
    def send_message_to_uid(self, profile_ctx: dict, uid: str, timeout_sec: int) -> SendResult:
        ...
        # The automation scheduler relies on a Qt timer that only fires when an
        # event loop is running in the owning thread. Because the worker runs
        # inside a background QThread without an active event loop, trigger the
        # first attempt manually so navigation feedback is immediate even before
        # the periodic timer becomes active.
        try:
            self._automation.attempt_typing()
        except Exception:
            # Any exception during the eager attempt will be surfaced by the
            # automation callback below, so we swallow it here to keep the
            # worker resilient.
            pass
        waited = 0
